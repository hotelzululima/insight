...
public:
  ...
  virtual Expr *rewrite (const Expr *F) {
	...
	if (! ctx->get_memory ()->is_defined (rdesc))
	  {
	    ...
	    ConcreteValue unk = ConcreteValue::unknown_value (regsize);
	...
	if (c != NULL)
	  {
	    ...
	    if (i == size)
	      ...
	      {
		ConcreteValue v = ConcreteValue::unknown_value (size * 8);
    ...
    if (val.hasValue ())
      {
	ConcreteValue sv = 
	  ConcreteExprSemantics::extract_eval (val.getValue (), offset, size); 
...
s_expand_memcell_indexes_rec (class ExprSolver *solver,
			      ...
			      const ConcreteContext *ctx, const Expr *e, 
